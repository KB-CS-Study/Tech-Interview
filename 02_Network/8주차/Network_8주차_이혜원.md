...
| 특징       | TCP/UDP 포트 정보를 바탕으로 함                                        | TCP/UDP 정보는 물론 HTTP의 URI, FTP의 파일명, 쿠키 정보 등을 바탕으로 분산함             |
| 장점       | - L7 로드밸런서보다 가격 저렴<br>  - 데이터 내용을 복호화할 필요가 없어 안전<br>  - 패킷 레벨 분산으로 속도가 빠르고 효율이 높음 | - 더 섬세한 라우팅 가능<br>  - 캐싱 기능 제공<br>  - 비정상 트래픽 필터링으로 안정성 높음     |
| 단점       | - 패킷 내용을 살펴볼 수 없어 정교한 라우팅 불가<br>  - IP 변경 시 연속 서비스 제공 어려움     | - 패킷 복호화 필요 → 비용 증가<br>  - 인증서 공유로 보안 위험 존재 (중간자 공격 가능성 등) |
...
### 웹 서버와 WAS의 역할 비교
<img width="695" alt="image" src="https://github.com/user-attachments/assets/8d9c4070-2451-4705-84b4-adb22a18f6a6" />

| 구분 | 웹 서버 (Web Server) | WAS (Web Application Server) |
|------|------------------------|-------------------------------|
| 개념 | 정적 컨텐츠 제공을 위한 서버 (HTTP 요청 응답) | 동적 컨텐츠 처리를 위한 애플리케이션 서버 |
| 구성 요소 | - 하드웨어: 웹 서버가 설치된 컴퓨터<br>- 소프트웨어: HTTP 요청을 받아 정적 파일을 응답하는 프로그램 | - JSP, Servlet 등의 웹 애플리케이션 실행 환경을 제공하는 미들웨어 |
| 주요 역할 | - 정적 컨텐츠 제공 (HTML, CSS, JS, 이미지 등)<br>- 클라이언트 요청을 WAS로 전달 | - 동적 컨텐츠 처리 (DB 조회, 비즈니스 로직 수행)<br>- 트랜잭션, 스레드 관리<br>- 프로그램 실행 환경 제공 |
| 동작 방식 | 1. 클라이언트가 HTTP 요청<br>2. 정적 파일이면 직접 응답<br>3. 동적 요청이면 WAS로 전달 | 1. 웹 서버로부터 요청 전달 받음<br>2. 비즈니스 로직 처리<br>3. DB 접근 후 결과 반환 |
| 예시 소프트웨어 | Apache, Nginx, IIS | Tomcat, JBoss, Jeus, WebSphere 등 |
| 별칭 | 웹 서버 | 웹 컨테이너, 서블릿 컨테이너 |
| 특징 | - 빠름, 가벼움<br>- 정적 자원 처리에 최적화 | - 무겁지만 동적 처리에 강함<br>- 보안, 트랜잭션, 분산 환경 지원 |
| 결합 예시 | Nginx + Tomcat<br>Apache + JBoss 등 | 단독 실행도 가능하지만 보통 웹 서버와 함께 사용 |
