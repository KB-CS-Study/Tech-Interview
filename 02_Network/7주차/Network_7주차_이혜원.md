...
| **항목** | **IPv4** | **IPv6** |
| --- | --- | --- |
| **주소 구성** | 수동 또는 DHCP로 주소 구성 | 자동 구성 (Stateless) 및 DHCPv6, 주소 재사용 지원 |
| **중단 간 연결 무결성** | `NAT로 인해 직접 연결 어려움` | `종단 간 연결 가능 (NAT 불필요)` |
| **주소 공간** | 약 43억 개 (2³²) 주소 | 약 3.4 × 10³⁸ 개의 주소 공간 (2¹²⁸) |
| **암호화 및 인증** | 선택적으로 제공 (IPSec은 별도 구성 필요) | IPSec이 표준에 포함되어 기본 제공 |
| **주소 길이** | `32비트 (4바이트)` | `128비트 (16바이트)` |
| **주소 표현** | `십진수 4개, 점(.)으로 구분 → 예: 192.168.0.1` | `16진수 8개, 콜론(:)으로 구분 → 예: 2001:db8::1` |
| **패킷 흐름 식별** | 미지원 (QoS 구현 어려움) | 흐름 레이블 필드로 패킷 흐름 식별 가능 (QoS 용이) |
| **체크섬 필드** | 사용함 | 체크섬 없음 (상위 계층에서 처리) |
| **메시지 전송 방식** | 브로드캐스트 (Broadcast) 지원 | 멀티캐스트(Multicast), 애니캐스트(Anycast) 지원 (브로드캐스트 제거) |
| TTL(Hop 제한)
| Time To Live (**TTL**) 필드 사용 
단위는 홉(초기엔 `초`)
| Hop Limit 필드 사용
`홉 수` 기준, TTL과 동일한 역할
|
...
![image.png](attachment:13bdda8c-4929-47da-abd8-7e43d83afee9:image.png)
...
- **POST**와 **PUT**, **PATCH**의 차이점
  ...
  | **동작 방식** | 서버가 URI 생성 (클라이언트는 위치 모름) | `클라이언트가 리소스 위치를 알고 URI 지정` 
    → 기존 리소스 완전히 대체 | 지정한 필드만 수정, **나머지는 유지`(리소스 부분 변경)`** |
...
| --- | --- | --- |
| 의미 | 암호화 기반 인터넷 보안 프로토콜 | 전송 계층 보안
인터넷 상의 커뮤니케이션을 위한 개인정보와 데이터 보안을 용이하게 하기 위해 설계되었으며 널리 채택된 보안 프로토콜 |
...
![image.png](attachment:93939369-e812-4e80-8fb7-ae76b84b1833:image.png)
...
![image.png](attachment:522ac161-a812-42be-8b38-9c7f2aab22cb:image.png)
...
![image.png](attachment:472ebe0b-0dd0-4fd4-9b19-20362b8b2d75:image.png)
...
![image.png](attachment:ea36dbf0-ec83-44ad-b162-bd81e47f5a10:image.png)
